...
<?php if ( ! bp_group_has_requested_membership() ) : ?>
	<p>
		<?php
			...
			echo esc_html( sprintf( __( 'You are requesting to become a member of the group "%s".', 'buddypress' ), bp_get_group_name() ) );
		?>
	</p>

	<form action="<?php bp_group_form_action( 'request-membership' ); ?>" method="post" name="request-membership-form" id="request-membership-form" class="standard-form">
		<label for="group-request-membership-comments"><?php echo esc_html_e( 'Comments (optional)', 'buddypress' ); ?></label>
		...
		<div class="submit">
			<input type="submit" name="group-request-send" id="group-request-send" value="<?php echo esc_attr_x( 'Send Request', 'button', 'buddypress' ); ?>" />
		</div> <!-- by razor -->
